...
# Build information :
#   add the following lines in the ant build.xml file :

 	<target name="buildinfo">
  	    <tstamp>
  	        <format property="builtat" pattern="dd/MM/yyyy HH'h'mm z"/>
  	    </tstamp>        

  	    <propertyfile file="src/project.properties"
  	        comment="This file is automatically generated - DO NOT EDIT">        
  	        <entry key="buildtime" value="${builtat}"/>
  	    	<entry key="buildhost" value="${env.COMPUTERNAME}"/>
  	    	<entry key="builder" value="${user.name}"/>
  	    </propertyfile>
  	</target>
  	
# and make the following target (it maybe "init" depends on "buildinfo"
...
public class RunningContext {
	...
	public RunningContext(String name, String systemProperty, String defaultPropertyUrlName) {
		
		initRunningContext(name, systemProperty, null, defaultPropertyUrlName, null) ;
	}

	public RunningContext(String name, String systemProperty, String baseDir, String defaultPropertyUrlName) {
		
		String defaultPropFile = "file://" + baseDir + defaultPropertyUrlName ;
		initRunningContext(name, systemProperty, baseDir, defaultPropFile, null) ;
	}
	
	public RunningContext(String name, String systemProperty, String baseDir, String defaultPropertyUrlName, Handler customLogHandler) {
		
		String defaultPropFile = "file://" + baseDir + defaultPropertyUrlName ;
		initRunningContext(name, systemProperty, baseDir, defaultPropFile, customLogHandler) ;
	}
	
	private void initRunningContext(String name, String systemProperty, String baseDir, String defaultPropertyUrlName, Handler customLogHandler) {
		pLog = null;
		initializationDate = Instant.now() ;
		try {
			// get property file
			propsStorage = new PropertiesStorage(systemProperty, defaultPropertyUrlName);
			props = propsStorage.getAdvanced() ; 
			
			// get the project property file if any
			...
			PropertiesStorage propsProjectStorage = new PropertiesStorage(projectBuildPropertyFile);
